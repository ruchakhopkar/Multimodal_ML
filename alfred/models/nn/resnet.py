...
class Resnet18(object):
    ...
    def extract(self, x):
        ...
        actual_img = [out.detach().cpu().numpy()]
        #print(out.shape)
        scale = 2
        arr = np.asarray(x)
        ...
        for i in range(0,h,int(h/scale)):
                for j in range(0,w, int(w/scale)):
                    ...
                    cut = cut.resize((h,w))
                    out = self.model(self.transform(cut).unsqueeze(0).cuda())
                    actual_img.append(out.detach().cpu().numpy())
        #print('actual_img', np.concatenate(actual_img, axis = 0).shape)
        actual_img = np.concatenate(actual_img, axis = 0)
        actual_img = torch.tensor(actual_img).cuda()
        actual_img = actual_img.unsqueeze(0).unsqueeze(3)
        actual_img = actual_img.permute(0,2,1,3)
        #out = out.unsqueeze(2).unsqueeze(3)
        #print('actual_img', actual_img.shape)
